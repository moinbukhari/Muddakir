...
export default function WordSidebar({
  ...
  return (
    <motion.aside
      ...
      className="sticky flex h-fit w-[350px] flex-col gap-4 rounded-lg border border-slate-400 bg-slate-200 p-8 pb-4"
    >
      <button
      ...
      <div className="flex flex-col items-center justify-center rounded-lg border border-slate-500 bg-slate-300 py-10 px-5 font-noton text-[7rem] font-medium shadow-lg">
        <div className="mt-4">{word.arabic}</div>
        <h2 className="text-center text-xl font-mono font-medium">{word.transliteration}</h2>
      </div>
      
      <h3 className="relative text-center font-manrope text-2xl">
      ...
      <h2 className="text-center text-2xl font-manrope text-slate-700 font-medium">{word.translation}</h2>
    </motion.aside>
  );
}
